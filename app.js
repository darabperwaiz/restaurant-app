...
function render(data) {
  const item = data.map((item) => {
    return `<div class="menu-item col-12 col-md-6 col-lg-4">
      <div class="card">
        <img src="${item.img}" style="aspect-ratio: 16/9;" class="card-img-top" alt="${item.title}">
        <div class="card-body">
          <h5 class="card-title">${item.title}</h5>
          <h6 class="card-subtitle mb-2 text-muted">${item.price}</h6>
          <p class="card-text" style="height: 50px; overflow: hidden; hyphens: auto; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${item.description}</p>
...
search.addEventListener("keyup", (e) => {
  ...
  let tokens = searchTerm.toLowerCase().split(' ')
  if(tokens.length) {
    let searchTermRegex = new RegExp(tokens.join('|'));
    let filteredList = menu.filter(function(item){
      let itemTitle = item.title.toString().toLowerCase().trim()
      return itemTitle.match(searchTermRegex)
    })
    render(filteredList)
  }
...
function filter(filter) {
  const filteredItems = menu.filter((item) => item.category == filter)
...
NavbarNav.addEventListener('click', (e) => {
  ...
  if(filterValue == 'starters' || filterValue == 'mains' || filterValue == 'desserts' || filterValue == 'beverages') {
    const filteredItems = filter(filterValue)
